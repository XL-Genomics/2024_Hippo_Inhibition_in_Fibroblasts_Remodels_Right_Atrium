...
####~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
####  Immune cell annotation by MNN  ####
####~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
sub.srt <- RunUMAP(sub.srt, reduction = 'mnn', dims = 1:50, min.dist = 0.6,
...
sub.srt$tmp[! sub.srt$Cell_type_prev %in% c('Immune', 'New', 'Doublet')] <- 'Other'
...
PlotPDF('1.2.feat.markers_immune', 10, 10)
FeaturePlot2(sub.srt, features = markers_lvl2_immune, reduction = 'sub_RNA_umap', raster = F)
dev.off()

Idents(sub.srt) <- 'CBN_snn_res.0.3'
mk <- FindAllMarkers(sub.srt, only.pos = T, logfc.threshold = 1, assay = 'CBN')
...
sub.srt$Cell_state[sub.srt$CBN_snn_res.0.2 %in% c(1)] <- 'MP1-Recruit' # Ccr2
sub.srt$Cell_state[sub.srt$CBN_snn_res.0.2 %in% c(0)] <- 'MP2-Resident' # Lyve1
sub.srt$Cell_state[sub.srt$CBN_snn_res.0.2 %in% c(2)] <- 'MP3-Prol' # Proliferative
sub.srt$Cell_state[sub.srt$CBN_snn_res.0.3 %in% c(5)] <- 'TC' # Skap1
sub.srt$Cell_state[sub.srt$CBN_snn_res.0.3 %in% c(6)] <- 'BC' # Cd79a
sub.srt$Cell_state[sub.srt$CBN_snn_res.0.3 %in% c(7)] <- 'DC' # Xcr1
sub.srt$Cell_state[sub.srt$CBN_snn_res.0.3 %in% c(4)] <- 'Doublet'
sub.srt$Cell_state[sub.srt$CBN_snn_res.0.6 %in% c(13)] <- 'Doublet'
sub.srt$Cell_state[sub.srt$Doublet_SC] <- 'Doublet'
sub.srt$Cell_state[is.na(sub.srt$Cell_state)] <- 'Doublet'

sub.srt$Cell_state <- factor(sub.srt$Cell_state, levels = c('MP1-Recruit', 'MP2-Resident', 'MP3-Prol',
                                                            'DC', 'TC', 'BC', 'Doublet'))
...
DimPlot2(sub.srt, reduction = 'sub_RNA_umap', label = T, cols = c(mycol_10[1:7], 'grey90'), group.by = 'Cell_state')
dev.off()
####~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~----
...
## Re-embed RNA
tmp.srt <- RunUMAP(tmp.srt, reduction = 'mnn', dims = 1:50, min.dist = 2,
...
p
PlotPDF('6.bar.cell_state_composition', 5, 5)
p
dev.off()
####~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~----
saveRDS(sub.srt, 'integrated/PART11.immune_cells.srt.rds')
####~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~----
