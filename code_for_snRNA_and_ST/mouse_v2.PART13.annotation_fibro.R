...
sub.srt <- srt[, srt$Cell_type %in% c('FB')]
...
####~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
####  FB cell annotation by WNN  ####
####~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
sub.srt <- RunUMAP(sub.srt, reduction = 'mnn', dims = 1:50, min.dist = 0.8,
...
sub.srt$tmp[! sub.srt$Cell_type_prev %in% c('FB', 'New', 'Doublet')] <- 'Other'
...
sub.srt <- AddModuleScore2(sub.srt, features = list(Yap_target$Yap_target_cf_cutrun), names = 'YapScore', 
                           return_z = T, assay = 'CBN')
PlotPDF('1.2.feat.markers_prev', 10, 10)
FeaturePlot2(sub.srt,  features = c('Tcf21', 'eGFP', 'creERT', 'YapScore'), reduction = 'sub_RNA_umap', raster = F)
dev.off()
...
sub.srt$Cell_state[sub.srt$CBN_snn_res.0.2 %in% c(0)] <- 'FB2' ## Gsn
sub.srt$Cell_state[sub.srt$CBN_snn_res.0.2 %in% c(1)] <- 'FB1' ## Pi16  
sub.srt$Cell_state[sub.srt$CBN_snn_res.0.2 %in% c(4)] <- 'FB3' ## Postn
sub.srt$Cell_state[sub.srt$CBN_snn_res.0.2 %in% c(2)] <- 'FB4-YapHi'
sub.srt$Cell_state[sub.srt$CBN_snn_res.0.2 %in% c(3, 6, 5)] <- 'Doublet'
sub.srt$Cell_state[sub.srt$Doublet_SC] <- 'Doublet'
sub.srt$Cell_state <- factor(sub.srt$Cell_state, levels = c('FB1', 'FB2', 'FB3', 'FB4-YapHi', 'Doublet'))
...
## Re-embed RNA
tmp.srt <- RunUMAP(tmp.srt, reduction = 'mnn', dims = 1:50, min.dist = 1,
...
dev.off()
####~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~----
saveRDS(sub.srt, 'integrated/PART13.fb_cells.srt.rds')
####~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~----
