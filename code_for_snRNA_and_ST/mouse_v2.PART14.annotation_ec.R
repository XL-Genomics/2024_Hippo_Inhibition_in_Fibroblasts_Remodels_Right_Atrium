...
sub.srt <- srt[, srt$Cell_type %in% c('EC')]
...
sub.srt$tmp[! sub.srt$Cell_type_prev %in% c('EC', 'New', 'Doublet')] <- 'Other'
...
PlotPDF('1.2.feat.known_endo_markers', 20, 20)
FeaturePlot2(sub.srt, features = c('Smad6', 'Efbn2', 'Nebl',
                                   'Car4', 'Rgcc', 'Pkd1l1',
                                   'Lyst', 'Ackr1', 'Nrg3',
                                   'Ccl21', 'Tbx1', 'Prox1', 
                                   'Chd11', 'Npr3', 'Plvap', 
                                   'Nrg1', 'Igfbp5'), reduction = 'sub_RNA_umap', raster = F, ncol = 3)
dev.off()

Idents(sub.srt) <- 'CBN_snn_res.0.2'
...
sub.srt$Cell_state[sub.srt$CBN_snn_res.0.2 %in% c(0)] <- 'EC2-LAEndoC' 
sub.srt$Cell_state[sub.srt$CBN_snn_res.0.2 %in% c(1)] <- 'EC3-RAEndoC' 
sub.srt$Cell_state[sub.srt$CBN_snn_res.0.2 %in% c(2)] <- 'EC1-Vessel'
sub.srt$Cell_state[sub.srt$CBN_snn_res.0.2 %in% c(5)] <- 'EC4-Prol'
sub.srt$Cell_state[sub.srt$CBN_snn_res.0.2 %in% c(4)] <- 'EC5-LEC'
sub.srt$Cell_state[sub.srt$CBN_snn_res.0.2 %in% c(3)] <- 'Doublet'
sub.srt$Cell_state <- factor(sub.srt$Cell_state, levels = c('EC1-Vessel', 'EC2-LAEndoC', 'EC3-RAEndoC',  
                                                            'EC4-Prol', 'EC5-LEC', 'Doublet'))
...
PlotPDF('3.dim.sub_cluster_annotated_wnn', 4, 6)
DimPlot2(sub.srt, reduction = 'sub_RNA_umap', label = T, cols = mycol_10, group.by = 'Cell_state')
dev.off()
####~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~----
...
## Re-embed RNA
tmp.srt <- RunUMAP(tmp.srt, reduction = 'mnn', dims = 1:50, min.dist = 0.8,
...
dev.off()
####~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~----
saveRDS(sub.srt, 'integrated/PART14.ec_cells.srt.rds')
####~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~----
